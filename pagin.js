...
const infoOnPage=[
  ...
  let infoLength = infoOnPage.length;
function gotoRespectivePage(ele){
    if(ele == "First"){
        createPagiNav();
    }
    displayContent(ele);
}
// create page number with previous,next, first and last button
function createPagiNav(){
    // create ul
    let ul_list = document.createElement("ul");
    ul_list.setAttribute("id", "listparent");
    ul_list.setAttribute("class", "pagination justify-content-center listparent");
    let infoPerPAge = 10;
    for(let i=0;i<infoLength/infoPerPAge; i++){
        let li_num = createLi("li");
        li_num.setAttribute("id","page"+(i+1));
        li_num.innerHTML = `<a class="page-link" href="#" onclick="gotoRespectivePage(this)">${i+1}</a>`;
        ul_list.appendChild(li_num);
    }
    let mainLi = document.getElementById("numList");
    mainLi.appendChild(ul_list);
    
}
function createLi(tagname){
    let listEle = document.createElement(tagname);
    listEle.setAttribute("class","page-item");
    return listEle;
}

function displayContent(eleInfo){
    let limitToDisplay = 10;
    let pageInfo = 1;
    let sIndex,eIndex=0;
    let pageNo = document.querySelector(".pageNo");
    const displayContent = document.getElementById("contentDiv");
    displayContent.innerHTML="";
    if(eleInfo == "First"){
        pageInfo=1;
    }
    else if(eleInfo == "Previous"){
        pageInfo = parseInt(pageNo.innerHTML)-1;
    }
    else if(eleInfo == "Next"){
        pageInfo=1;
        pageInfo = parseInt(pageNo.innerHTML)+1;
    }
    else if(eleInfo == "Last"){
        pageInfo=infoLength/limitToDisplay;
    }
    else {
        pageInfo=parseInt(eleInfo.innerHTML); 
    }
    
    pageNo.innerText = pageInfo;
    eIndex = pageInfo*limitToDisplay;
    sIndex = eIndex - limitToDisplay;
    document.getElementById("pagefromto").innerText = (sIndex+1)+" - "+eIndex;
    for(let i=sIndex;i<eIndex; i++){
        let infoEle = document.createElement("div");
        infoEle.setAttribute("class", "infoElemDiv");
        infoEle.innerHTML += `<b>Id:&nbsp;</b>${infoOnPage[i].id}<br><b>Name:&nbsp;</b>${infoOnPage[i].name}<br><b>Email:&nbsp;</b>${infoOnPage[i].email}`;
        displayContent.appendChild(infoEle);
    }
    disabledOthers(pageInfo,limitToDisplay);
    if(pageInfo === 1)
    {
        // disable previous and enable next
        document.getElementById("prevPage").setAttribute("class","page-item disabled");
        document.getElementById("firstPage").setAttribute("class","page-item disabled");
        document.getElementById("lastPage").setAttribute("class","page-item enabled");
        document.getElementById("nextPage").setAttribute("class","page-item enabled");
    }
    else if(pageInfo == limitToDisplay){
        document.getElementById("nextPage").setAttribute("class","page-item disabled");
        document.getElementById("lastPage").setAttribute("class","page-item disabled");
        document.getElementById("prevPage").setAttribute("class","page-item enabled");
        document.getElementById("firstPage").setAttribute("class","page-item enabled");
    }
    else{
        document.getElementById("prevPage").setAttribute("class","page-item enabled");
        document.getElementById("firstPage").setAttribute("class","page-item enabled");
...
function disabledOthers(enableNo, disCount){
    for(let i=1;i<=disCount; i++){
        let deselectedLi = document.getElementById("page"+i);
        let deselectedA = deselectedLi.querySelector(".page-link");
        deselectedA.setAttribute("class","page-link");
    }
    let selectedLi = document.getElementById("page"+enableNo);
    let selectedA = selectedLi.querySelector(".page-link");
    selectedA.setAttribute("class","page-link selectedItem");
}